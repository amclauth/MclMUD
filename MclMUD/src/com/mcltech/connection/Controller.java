   ...
   public void run()
   {
      try (InputStream in = client.getInputStream())
      {
         ...
         do
         {
            ...
            if (in.available() == 0)
            {
               AnsiParser.getInstance().flush();
            }
            ret_read = in.read(buff);
            if (ret_read > 0)
            {
               AnsiParser.getInstance().parseBytes(buff, ret_read);
         ...
         in.close();
         isConnected = false;
         MudFrame.getInstance().writeToTextBox("Disconnected.", null);
      }
      catch (IOException e)
      {
